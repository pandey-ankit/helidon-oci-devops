...
1. Open a new terminal in the OCI Code Editor and paste the following command to navigate to home folder.
    ```bash
    <copy>cd ~</copy>
    ```
...
1. Execute the cli to generate a Helidon Microprofile application project.
    ...
    ```
2. When it prompts for *Helidon version*, copy and paste the below value to the terminal.
    ```bash
...
3. Open a new terminal, Copy and paste the following command to copy the build and deployment pipeline specs from the *`devops_helidon_to_instance_ocw_hol`* repository.
    ```bash
...
5. Copy and paste the following command to run the utility script from the main repository *`devops_helidon_to_instance_ocw_hol`* to update the config parameters. when it ask for *Enter the Helidon MP project's root directory, press enter to select the default value.
    ```bash
    <copy>~/devops_helidon_to_instance_ocw_hol/utils/update_config_values.sh</copy>
    ```
    You will have output similar to shown below:
...
In this step, we are going to generate an *Authentication Token*, that we will use to push the Helidon application code to *OCI Code repository*.
...
5. Configure git to use credential helper store so that oci repository's username and password will be entered only once on git commands that require them. Also, set *user.name* and *user.email* which is required by git commit.
    ```bash
...
7. This will prompt for username and password. Use <tenancy name>/<username> for the username and oci user auth token that was generated at task 4 for the password.
...
9. Stage all the files for git commit.
    ```bash
    <copy>git add .
    git status</copy>
    ```
...
3. Select the compartment, which you have create in **Lab 1** and then Click *devops-project-helidon-ocw-hol-string* to open the DevOps Project.
    ![select compartment](images/select-compartment.png)

4. Under *Latest build history*, you will see the *runs* and Status as *Accepted*. Click on latest runs as shown below.
    ![build history](images/build-history.png)

5. Once the build pipeline successfully completed all the three stages, you will see output as shown below.
    ![build run first](images/build-run-first.png)

6. In Build run progress, In third stage, click on Three dots and then click *View deployment* as shown below. This will open deployment pipeline. 
    ![view deployment](images/view-deployment.png)

7. Here you can see *Deployment Progress*. Once you have deployment pipeline successfully completed, you will see output as shown below.
    ![deployment run](images/deployment-run.png)

    > This successfully deploy the Helidon application to Compute instance in OCI.

8. To view the logs of deployment pipeline, click on *Three dots* near to deployment stage and click *View details* as shown below.
...
9. Scroll down the logs and verify the JDK flavour, It should be Open JDK as shown below.
    ...
    > As part of Lab 4, we will replace Open JDK with Oracle JDK. This logs verify that you are currently using Open JDK 20. 
